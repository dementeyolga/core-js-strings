...
function removeFirstOccurrences(str, value) {
  ...
  return occurrence === -1
    ? str
    : str.slice(0, occurrence) + str.slice(occurrence + value.length);
}
...
function removeLastOccurrences(str, value) {
  ...
  return lastOccurrence === -1
    ? str
    : str.slice(0, lastOccurrence) + str.slice(lastOccurrence + value.length);
}
...
function isPalindrome(str) {
  const withoutMarks = str.replaceAll(/[?!, ]/g, '');
  return (
    withoutMarks.toLowerCase() ===
    withoutMarks.toLowerCase().split('').reverse().join('')
  );
}
